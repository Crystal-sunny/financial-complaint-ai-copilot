...
V6 在首次模型请求前冻结，12 个样本全部端到端完成，36 个 Agent 阶段输出均通过结构化校验。75.0% 是当前合成冻结集的内部工程观测，不是生产准确率、独立基准或金融业务承诺。详见 [评测口径](docs/EVALUATION.md)。
...
evals/               冻结评测集、SHA 锁与运行清单
scripts/             离线测试、真实模型评测和汇总脚本
docs/                架构与评测口径
```
