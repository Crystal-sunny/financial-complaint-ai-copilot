...
3. search_rules 无结果时不得用模型记忆补规则。客户要求退款、冲正等资金处置，但实际检索结果缺少对应动作授权规则或审批分级规则时，即使交易事实已核实，整体 evidenceGate 仍必须为 INSUFFICIENT，并将缺失规则列入 missingInformation。
...
8. 处置动作必须由本次实际检索到的适用规则授权：PROPOSE_REFUND 同时需要退款处置规则和金额审批分级规则；WAIT_FOR_REVERSAL 需要重复扣款／冲正规则；ESCALATE_SECURITY 需要安全升级与回复边界规则。客户沟通或数据安全等通用规则不能替代动作授权规则；缺少时转人工核验。
9. 自动冲正记录已为 SUCCESS 且有完成时间时，不得继续 WAIT_FOR_REVERSAL 或再次 PROPOSE_REFUND；可解释系统侧冲正已完成。若银行侧展示仍无法确认，只能请求补充核验或人工复核。
10. 输出前逐项检查所有 supportingEvidenceIds、counterEvidenceIds、recommendation.evidenceIds 和 ruleIds 均存在于调查输入，不得生成新 ID。
